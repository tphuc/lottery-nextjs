import { amber, amberDark, blackA, indigo, mauve, orange, orangeDark, purple, violet, whiteA, yellow } from '@radix-ui/colors'
    ...
    <Container>
      ...
      <Box css={{ textAlign: "center", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
        ...
        <Box css={{ width: 500, maxWidth: "90vw", padding: 20, borderRadius: 12, background: `linear-gradient(125deg, ${purple.purple9} 0%, ${violet.violet10} 74%)`, }}>
          ...
          {!isAvailableToJoin && <h4 style={{fontSize:20}}>Prize pool: {ticketPrice*participants} ETH </h4>}
        ...
        <Box css={{fontSize:"2em", color:yellow.yellow10, padding:10, border:"1px solid rgba(255,255,255,0.2)", borderRadius:10}}>
          {balance?.toString() && <span>{ethers.utils.formatEther(balance?.toString())} ETH</span>}
        </Box>
        <br />
        <h2>History</h2>
        <Box css={{
      ...
      <Box css={{ paddingLeft: '10%', paddingRight: "10%" }}>
        <Box id='about'>
          <h4 style={{ fontSize:'2em', color: violet.violet5 }}>About</h4>
          <p style={{ fontSize: "1.2em", fontWeight: 300 }}>Krypto Lottery is the digitized version of the traditional lottery system. Secure, transparent, randomness verifiable living on Ethereum blockchain.</p>
        </Box>
        <Box id='howtoplay'>
          <h4 style={{ fontSize:'2em', color: violet.violet5 }}>How to play</h4>
          <p style={{ fontSize: "1.2em", fontWeight: 300 }}>When you buy a lottery ticket on our website, you’re placing a bet on what you think the winning numbers of a particular national lottery draw will be. You are not buying a ticket to the actual lottery itself. This makes us totally transparent, as we have no influence on the winning numbers.

            If the numbers on your ticket match those drawn, you win a prize from us. The prizes are shown before every draw in the prize table on the Play Now pages and vary in size depending on how many numbers you guess correctly. We guarantee all prizes and pay outs immediately after each draw.</p>
        </Box>
        <p></p>
      </Box>
